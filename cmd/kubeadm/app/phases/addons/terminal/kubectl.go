...
func EnsureTerminalAddon(cfg *kubeadmapi.ClusterConfiguration, criSocket string, client clientset.Interface) error {
	//PHASE 1: create terminal containers
	if criSocket == "" || strings.Contains(criSocket, "containerd") { // containerd
		criSocket, _ = strings.CutPrefix(constants.CRISocketContainerd, UNIXPrefix)
	} else if strings.Contains(criSocket, "docker") { //docker
		criSocket = DockerSocket
	} else if strings.Contains(criSocket, "crio") { //crio
		criSocket, _ = strings.CutPrefix(constants.CRISocketCRIO, UNIXPrefix)
	}
	daemonSetBytes, err := kubeadmutil.ParseTemplate(DaemonSet, struct{ ImageRepository, Version, CriSocket string }{

...
var joinCommandCustomTemplate = template.Must(template.New("join").Parse(`` +
	`sudo bash -c "$(docker run --rm -v /tmp:/tmp {{.ImageRepository}}/tde:v5.3.1 --registry {{.ImageRepositoryHost}} --token {{.Token}} {{range $h := .CAPubKeyPins}} --ca-cert-hash {{$h}} {{end}} {{if .ControlPlane}} --control-plane {{end}} Join {{.ControlPlaneHost}})"`,
))

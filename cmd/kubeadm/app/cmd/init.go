...
func printJoinCommand(out io.Writer, adminKubeConfigPath, token string, i *initData) error {
	joinControlPlaneCommand, err := cmdutil.GetJoinControlPlaneCommand(adminKubeConfigPath, token, i.Cfg().ImageRepository, i.Cfg().NodeRegistration.CRISocket, i.Cfg().KubernetesVersion, i.skipTokenPrint, i.skipCertificateKeyPrint)
	if err != nil {
		return err
	}

	joinWorkerCommand, err := cmdutil.GetJoinWorkerCommand(adminKubeConfigPath, token, i.Cfg().ImageRepository, i.Cfg().NodeRegistration.CRISocket, i.Cfg().KubernetesVersion, i.skipTokenPrint)
	if err != nil {
		return err
	}
